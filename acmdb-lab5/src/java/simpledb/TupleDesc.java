...
public class TupleDesc implements Serializable {
    ...
    private class TDItemIterator implements Iterator<TDItem> {
        ...
        public boolean hasNext() {
            return now<tupleDesc.length;
        }
        @Override
        public TDItem next() {
            return tupleDesc[now++];
        }
    }
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        if(name != null) 
            for (int i=0;i<tupleDesc.length;i++)
                if (name.equals(tupleDesc[i].fieldName))return i;
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        ...
        for(int i=0; i<m; i++) {
            fieldType[n+i]=td2.getFieldType(i);
            fieldName[n+i]=td2.getFieldName(i);
        }
