...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreePageId nextPage=null;
		if(f==null) nextPage=it.next().getLeftChild();
		else {
			BTreeEntry entry=it.next();
			while (f.compare(Op.GREATER_THAN,entry.getKey())&&it.hasNext())
				entry=it.next();
			if (f.compare(Op.LESS_THAN_OR_EQ, entry.getKey()))nextPage=entry.getLeftChild();
			else nextPage=entry.getRightChild();
		}
		return findLeafPage(tid,dirtypages,nextPage,perm,f);
	}
	/**
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		//get a new Page with half tuple of the old page
		BTreeLeafPage newPage=(BTreeLeafPage)getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		int numToMove=page.getNumTuples()/2;//Tuples is even, directly num/2
		...
		//change the points of affected leaf pages
		BTreePageId RightPagePid=page.getRightSiblingId();
		newPage.setRightSiblingId(RightPagePid);
		newPage.setLeftSiblingId(page.getId());
		page.setRightSiblingId(newPage.getId());
		if(RightPagePid!=null) {
			BTreeLeafPage RightPage=(BTreeLeafPage)getPage(tid, dirtypages, RightPagePid, Permissions.READ_WRITE);
			RightPage.setLeftSiblingId(newPage.getId());
			dirtypages.put(RightPagePid,RightPage);
		}
		...
		//return the page that new tuple inserted
		if(field.compare(Op.LESS_THAN_OR_EQ, middleKey)) return page;
	 	else return newPage;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//get a new InternalPage
		BTreeInternalPage newPage=(BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		int numEntriesToMove=(page.getNumEntries()-1)/2;//the num of entry is odd, so need (EntryNum-1)/2
		Iterator<BTreeEntry> it=page.reverseIterator();
		for(;numEntriesToMove>0;numEntriesToMove--){
			BTreeEntry entry=it.next();
			page.deleteKeyAndRightChild(entry);
			newPage.insertEntry(entry);
		}
		
		//middleEntry will be inserted to parent InternalPage
		...
		page.setParentId(parent.getId());
		newPage.setParentId(parent.getId());
		//uodate new Page's child page's pointers to newPage
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		for (;numTuplesToMove>0;numTuplesToMove--){
			Tuple tuple = it.next();
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//get the number of entry to move
		...
		for(;numEntriesToMove>0;numEntriesToMove--){
			...
			page.insertEntry(new BTreeEntry(middleKey,leftEntry.getRightChild(),rightEntry.getLeftChild()));
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//delete ParentEntry and insert to leftpage
		this.deleteParentEntry(tid,dirtypages,leftPage,parent,parentEntry);
		...
		//update the rightEntry to left Page
