...
public class BufferPool {
    ...
    public void insertTuple(TransactionId tid, int tableId, Tuple t)
        ...
        for(Page dirtyPage : dirtyPages) {
            ...
            pid2page.replace(dirtyPage.getId(), dirtyPage);
        }
    ...
    */
    ...
    private synchronized  void evictPage() throws DbException {
        // some code goes here
        // not necessary for lab1
        PageId pid=pid2page.keySet().iterator().next();//evict the first Page in pid2page
        try {
            flushPage(pid);
            discardPage(pid);
        } catch (IOException e) {
            e.printStackTrace(); //or writepage error
        }
    }
