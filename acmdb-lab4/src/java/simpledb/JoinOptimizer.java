...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int bigger=Math.max(card1, card2);
        switch (joinOp) {
            case EQUALS:
                if (t1pkey && t2pkey) {
                    card = smaller;
            ...
            case NOT_EQUALS:
                if(t1pkey && t2pkey) {
                    card=card1*card2-smaller;
                } else if (t1pkey) {
                    card=card1*card2-card2;
                } else if (t2pkey) {
                    card=card1*card2-card1;
                } else {
                    card=card1*card2-bigger;
                }
                break;
            case GREATER_THAN:
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        //also by the guide of docs
        int numJoin=this.joins.size();
        PlanCache pl = new PlanCache();
        for (int i=1;i<= numJoin;i++) {
            ...
            for (Set<LogicalJoinNode> subset : setOfSubset) {
                ...
                for (LogicalJoinNode toRemove : subset) {
                    CostCard plan=this.computeCostAndCardOfSubplan(stats,filterSelectivities,toRemove,subset,bestCostSofar,pl);
                    if (plan != null && plan.cost < bestCostSofar) {
                        bestPlan=plan;
                        bestCostSofar=plan.cost;
                    }
                }
                pl.addPlan(subset,bestPlan.cost,bestPlan.card,bestPlan.plan);
            }
        }
        Set<LogicalJoinNode> wholeSet = this.enumerateSubsets(this.joins, numJoin).iterator().next();
        if (explain) 
            this.printJoins(this.joins, pl,stats,filterSelectivities);
        return pl.bestOrders.get(wholeSet);
